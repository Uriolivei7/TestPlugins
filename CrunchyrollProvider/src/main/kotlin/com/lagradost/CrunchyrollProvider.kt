...
//                        .forEach {
                    callback(
                        newExtractorLink(
                            ...
                            url = stream.url,
                            // type: ExtractorLinkType? = null (no lo especificamos si no es necesario un tipo especial,
                            // o puedes usar ExtractorLinkType.M3U8 para m3u8 si lo deseas)
                            // Puedes omitir 'type' si es null por defecto
                        ) { // Inicia la lambda initializer
                            quality = getQualityFromName(stream.resolution)
                            // Si necesitas headers o referer, irían aquí también:
                            // headers = yourHeadersMap
                            // referer = yourRefererString
                        }
                    )
//                        }
                    ...
                    callback(
                        newExtractorLink(
                            ...
                            name = "Crunchy - ${stream.title} ★",
                            url = premiumStream,
                            // type: ExtractorLinkType? = null (omisible)
                        ) { // Inicia la lambda initializer
